...
def runFlopAction(log, start, action, players, table, betThisRound, betToMatch, currAction, option, actionExps, flopExp, turnExp, cardExp):
    for i in range(start, len(log)):
        line = log[i]

        if flopExp.search(line) != None:
            print("flop")
            round = "flop"
            currAction = None
            for c in (cardExp.findall(line)):
                flopCard = Card(c)
